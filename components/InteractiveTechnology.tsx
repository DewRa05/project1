...
export default function InteractiveTechnology() {
  ...
  return (
    <section id="8" className="bg-white py-6 sm:py-8 lg:py-12 px-4 sm:px-6 lg:px-10">
      <h2
        className="text-center text-[#01B3BF] text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-6 sm:mb-8 font-extrabold leading-snug"
        data-aos="fade-up"
        data-aos-delay="100"
      >
      ...
      <div className="py-4 sm:py-6 lg:py-8 h-fit">
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 lg:gap-8 items-start">
          ...
          <section className="w-full lg:w-2/3" data-aos="fade-up" data-aos-delay="200">
            <iframe
              ...
              scrolling="no"
              src="/second.html"
              className="inset-0 w-full border-0 h-[40vh] sm:h-[55vh] lg:h-[97vh] overflow-hidden"
            ></iframe>
          </section>

          {/* Sidebar */}
          <ul
            className="w-full sm:max-w-md lg:w-[280px] h-fit space-y-1 sm:space-y-2 border border-gray-300 rounded-lg shadow-md bg-white p-1 sm:p-2 mt-2 sm:mt-0"
            data-aos="fade-up"
            data-aos-delay="300"
          >
            {areaLabels.map((label, i) => (
              <li
                key={i}
                className="sidebar-item cursor-pointer px-2 py-1 sm:px-3 sm:py-2 rounded-md text-gray-700 font-normal text-xs sm:text-sm hover:bg-[#effffb] hover:text-[#00c990] hover:border-l-4 hover:border-[#00b7d4] transition"
                data-area={i + 1}
              >
                {label}
              </li>
